  ...
  checkEligibility(input: ScreeningInput) {
    ...
    if (annualIncome > roughAMI50 * 1.6) {
      return {
        eligible: false,
        confidence: 'low',
        reason: 'Income likely exceeds 80% of Area Median Income. Limits vary significantly by location.',
      };
    }
    ...
    const hasChildren = input.householdMembers.some((m) => m.relationship === 'child');

    const rentEstimate = input.monthlyExpenses?.rent ?? 0;
    const rentBurden = input.monthlyIncome > 0 ? rentEstimate / input.monthlyIncome : 0;

    return {
      eligible: true,
      confidence: 'low',
      reason: 'You may qualify based on household income. Waitlists can be long — apply early.',
      estimatedMonthlyValue: rentEstimate > 0 ? Math.round(rentEstimate * 0.5) : 500,
      estimatedAnnualValue: (rentEstimate > 0 ? Math.round(rentEstimate * 0.5) : 500) * 12,
      nextSteps: [
        ...
        rentBurden > 0.5
          ...
          : '',
        hasElderlyOrDisabled ? 'Elderly/disabled households may qualify for preference' : '',
