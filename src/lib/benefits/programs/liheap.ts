  ...
  checkEligibility(input: ScreeningInput) {
    ...
    if (!isBelowFPLPercent(annualIncome, input.householdSize, 150)) {
      return {
        eligible: false,
        confidence: 'medium',
        reason: 'Income exceeds 150% FPL. Some states use higher thresholds — check your state.',
      };
    }
    ...
    return {
      ...
      nextSteps: [
        ...
        hasElderlyOrDisabled ? 'Mention elderly/disabled status — you may get priority' : '',
      ].filter(Boolean),
    };
  },
};
