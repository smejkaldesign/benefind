...
export const medicaid: BenefitProgram = {
  ...
  checkEligibility(input: ScreeningInput) {
    ...
    if (hasPregnant && isBelowFPLPercent(annualIncome, input.householdSize, 200)) {
      return {
        eligible: true,
        confidence: 'high',
        reason: 'Pregnant women in most states qualify for Medicaid at up to 200% FPL.',
    ...
    if (hasChildren && isBelowFPLPercent(annualIncome, input.householdSize, 200)) {
      return {
        eligible: true,
        confidence: 'high',
        reason: 'Children under 19 typically qualify at higher income levels than adults.',
    ...
    if (isBelowFPLPercent(annualIncome, input.householdSize, 138)) {
      return {
        eligible: true,
        confidence: 'medium',
        reason: 'Your income is below 138% FPL. Most states have expanded Medicaid to this level.',
        estimatedMonthlyValue: 500,
        estimatedAnnualValue: 6000,
        nextSteps: [
    ...
    return {
      eligible: false,
      confidence: 'medium',
      reason: 'Income appears above Medicaid limits, but eligibility varies by state. Check ACA marketplace subsidies.',
    };
  },
};
