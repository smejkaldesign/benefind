...
export const rndTaxCredit: CompanyProgram = {
  ...
  checkEligibility(input: CompanyScreeningInput) {
    ...
    const whyYouQualify: string[] = [
      'You spend on R&D or product development',
    ];
    if (isSmall) whyYouQualify.push('You have fewer than 500 employees');
    if (isStartup) whyYouQualify.push('As a startup (< 5 years), you can offset up to $250K in payroll taxes');
    ...
    return {
      ...
      matchScore: 95,
      confidence: 'high',
      reason: 'Companies spending on R&D, product development, or process improvement qualify for this permanent federal credit.',
      ...
      nextSteps: [
        ...
        ...(isStartup ? ['File Form 8974 to offset payroll taxes (startup benefit)'] : []),
      ],
      whyYouQualify,
    };
  },
